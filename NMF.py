...
def plotNMFSpectra(V, W, H, iter, errs, hopLength = -1):
    ...
    if hopLength > -1:
        ...
        plt.imshow(V, cmap = 'magma', interpolation = 'none', aspect = 'auto')
    plt.title("V")
    plt.subplot(223)
    WH = W.dot(H)
    if hopLength > -1:
        ...
        plt.imshow(WH, cmap = 'magma', interpolation = 'none', aspect = 'auto')
    plt.title("W*H Iteration %i"%iter)  
    plt.subplot(222)
    plt.imshow(np.log(H + np.min(H[H > 0])), cmap = 'magma', interpolation = 'none', aspect = 'auto')
...
def doNMFDriedger(V, W, L, r = 7, p = 10, c = 3, plotfn = None, plotfnw = None, plotEvery = 20):
    """
    ...
    for l in range(L):
        ...
        if plotfn and ((l+1)==L or (l+1)%plotEvery == 0):
            plt.clf()
